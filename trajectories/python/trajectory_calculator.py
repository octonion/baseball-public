...
#f20 = open('parameters.csv','r')
#csv20 = csv.reader(f20)

# Skip header
#next(csv20)
#row = next(csv20)
#cd0,cddot,cdspin,w0,theta0,tau0 = row
#print(row)
...
f21 = open('trajectory_calculator_output.csv','w')
csv21 = csv.writer(f21)
...
while (True):
    ...
    print()
    nopt = int(input(' enter option: '))

    if (nopt==0):
        sys.exit()
    elif (nopt < 1 or nopt > noptmax):
        sys.exit()
    elif (nopt < 5):
        print(' options 1-4 not yet implemented, choose another')
    else:
        if (nopt==5):
            trajectory_input = pd.read_csv('trajectory_input.csv')

            ndata = trajectory_input.shape[0]

            tmax = 10.0
            # 251 format(' rho, v0, theta, spin, sfact, cdi, vwind, distance, tof')

            #write(21,251) !write header
            for index, row in trajectory_input.iterrows():
                print(row)
                rho = row['rho']
                ...
                cphi = 1.0
                sphi = 0.0
                spin = (w0/rpm)*(v0/100.0)*(theta-theta0)/(27.5-theta0)
