...
ncaa_asf.each do |asf|
  ...
  if (stat_category=="hitting" and (split_name==split))
...
ncaa_player_summaries = CSV.open("csv/ncaa_player_summaries_hitting_#{year}_#{division}_#{split}.csv",
                                 "w",
                                 {:col_sep => "\t"})
