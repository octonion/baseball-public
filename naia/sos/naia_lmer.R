sink("diagnostics/naia_lmer.txt")
...
dbWriteTable(con,c("naia","_parameter_levels"),parameter_levels,row.names=TRUE)
...
model <- log_rs ~ year+field+(1|park)+(1|offense)+(1|defense)
...
dbWriteTable(con,c("naia","_basic_factors"),as.data.frame(combined),row.names=TRUE)
