...
from njcaa._parameter_levels
where parameter='year'
);

drop table if exists njcaa._factors;

create table njcaa._factors (
       parameter		text,
       level			text,
       type			text,
       method			text,
...
from njcaa._parameter_levels npl
left outer join njcaa._basic_factors nbf
