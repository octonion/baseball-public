...
from ncaa._zim_parameter_levels
where parameter='year'
);

drop table if exists ncaa._zim_factors;

create table ncaa._zim_factors (
       parameter		text,
       level			text,
       type			text,
       method			text,
...
from ncaa._zim_parameter_levels npl
left outer join ncaa._zim_basic_factors nbf
...
parameter,
avg(raw_factor)
from ncaa._zim_factors
where parameter not in ('h_div','p_div')
group by parameter
);

update ncaa._zim_factors
set raw_factor=raw_factor-s.mean
from scale s
where s.parameter=_zim_factors.parameter;
